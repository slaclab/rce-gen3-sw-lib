...
#define XML_iterate_open(pvar,tvar)                               \
  for(cm::XML::TagIterator it(pvar); !it.end(); it++) { \
    const cm::XML::StartTag& tvar = *it;                

#define XML_iterate_close(where,tvar)           \
  else                                          \
    cm::service::printv(#where " unknown tag %s/%s\n",        \
           tvar.element.c_str(),                \
           tvar.name.c_str());                  \
  }

#define XML_insert( p, element, name, routine ) {                  \
    XML::IO::insert(p, cm::XML::StartTag(element,name)); \
    { routine; }                                                   \
    XML::IO::insert(p, cm::XML::StopTag (element)); }
