...
void* FmTahoe::operator new(unsigned s, int id)
{
  void* p = (void*)(FM_DEV_BASE + id*FM_DEV_SIZE);
  return p;
}

void  FmTahoe::operator delete(void*)
{
}

void  FmTahoe::destroy()
{
  //  delete this;
}

unsigned FmTahoe::id() const
{
...
FmCpuPortStatus FmTahoe::cpuPortStatus()
{
  return FmCpuPortStatus(0);
}

void FmTahoe::resetChip()
{
  cm::svc::PlxBridge::instance().reset();
}

void FmTahoe::releaseChip()
{
}

int FmTahoe::txPacket(const unsigned* data,
		      unsigned   len,
...
int FmTahoe::rxPacket(unsigned* d)
{
  if (!(lci_status & FmTahoe::Lci_Status::RxRdy))
    return 0;
  ...
  bool ldone=false;
  while(!ldone) {
    ldone = (lci_status & FmTahoe::Lci_Status::RxEOT);
    if (!_poll(lci_status,
	       FmTahoe::Lci_Status::RxRdy,
	       FmTahoe::Lci_Status::RxRdy,
	       1024)) {
...
void FmTahoe::enableInterrupts(unsigned i)
{
}

void FmTahoe::disableInterrupts(unsigned i)
{
}

void FmTahoe::enable_tx_fifo_interrupt(unsigned id)
{
}

void FmTahoe::disable_tx_fifo_interrupt(unsigned id)
{
}

void FmTahoe::enable_rx_fifo_interrupt(unsigned id)
{
}

void FmTahoe::disable_rx_fifo_interrupt(unsigned id)
{
}

int FmTahoe::_poll(FmMReg& r,unsigned mask,unsigned val,unsigned timeout)
{
  volatile unsigned v;
...
unsigned FmTahoe::run_bist(FmPortMask pmask,
			   unsigned itest,
			   unsigned len,
			   unsigned* results)
{
  ...
  // check for sigdet
  list.read (&list.epl()->serdes_status, results);
