...
#include "control/DtmMonitorTask.hh"
...
static unsigned dtm_command;
static unsigned dtm_arg;
static unsigned dtm_arg2;

DtmMonitorTask::DtmMonitorTask(cm::net::Manager* m) :
  fm   (m)
{
}

DtmMonitorTask::~DtmMonitorTask()
{
}

void DtmMonitorTask::run()
{
  dtm_command = 0;
...
void DtmMonitorTask::_execute()
{
    ...
    case 7:
      { rwReg(); break; }
    default:
      { timespec tv; tv.tv_sec=2; nanosleep(&tv,0);
	break; }
    }
...
void DtmMonitorTask::basePhyMgmt()
{
...
void DtmMonitorTask::rxCheck()
{
  FmTahoe& _dev = fm->deviceMgr(0).dev();

  unsigned v0 = _dev.sys_cfg_3;
  unsigned v1 = _dev.sys_cfg_4;
  printf("sys_cfg_3/4 : %08x %08x\n", v0, v1);

  v0 = _dev.lci_cfg;
  v1 = _dev.lci_status;
  unsigned v2 = _dev.lci_ip;
  unsigned v3 = _dev.lci_im;

  printf("lci cfg/status : %08x %08x  ip/im : %08x %08x\n",
	 v0, v1, v2, v3 );

  asm volatile("mfdcr %0,%1" : "=r"(v0) : "i"(TAHOE_FIFO_IS_REG));
  asm volatile("mfdcr %0,%1" : "=r"(v1) : "i"(TAHOE_FIFO_IM_REG));
  asm volatile("mfdcr %0,%1" : "=r"(v2) : "i"(TAHOE_STAT_REG));
  asm volatile("mfdcr %0,%1" : "=r"(v3) : "i"(TAHOE_FIFO_RX_REG));

  printf("dcr ip/im/st %x:%x:%x  rxaddr %08x\n",
	 v0, v1, v2, v3 );
}
