/* $Id: xdma_multi_sg.c 1948 2013-08-15 18:48:49Z claus $ */
/******************************************************************************
*
...
int XDmaMulti_SgStart(XDmaMulti * InstancePtr, unsigned Channel)
{
	...
	if (XDmaMulti_IsSgListEmpty(InstancePtr, Channel) == TRUE) {
	...
	if ((LastDescriptorPtr == NULL)) {
		XIo_Out32(CHANNEL_REGS + XDM_BDA_REG_OFFSET,
			  (u32) CHANNEL_DATA.GetPtr);
		Register = XIo_In32(CHANNEL_REGS + XDM_SWCR_REG_OFFSET);
			   ...
			   unsigned Channel, u32 *BdMemoryPtr, u32 ByteCount)
{
	...
	XASSERT_NONVOID(InstancePtr != NULL);
	XASSERT_NONVOID(InstancePtr->ChannelCount > Channel);
	XASSERT_NONVOID(BdMemoryPtr != NULL);
	...
	while ((UsedByteCount + sizeof(XBufDescriptor)) <= ByteCount) {
		 ...
		 */
		BufferDescriptorPtr = (XBufDescriptor *) ((u32) BdMemoryPtr +
							  UsedByteCount);
...
*
* @return
* - TRUE if the scatter gather list is empty, otherwise a value of
*   FALSE.
*
* @note
*
			    ...
			    XBufDescriptor * BufferDescriptorPtr)
{
	 ...
	 * use this one yet.
	 */
	CopyBufferDescriptor(BufferDescriptorPtr, CHANNEL_DATA.PutPtr);
	...
	if ((CHANNEL_DATA.CommitPtr !=
	     CHANNEL_DATA.LastPtr) && (CHANNEL_DATA.CommitPtr != NULL)) {
	 ...
	 */
	if (CHANNEL_DATA.CommitPtr == NULL) {
...
int XDmaMulti_CommitPuts(XDmaMulti * InstancePtr, unsigned Channel)
{
	 ...
	 */
	if ((CHANNEL_DATA.CommitPtr == NULL) ||
	    XDmaMulti_IsSgListEmpty(InstancePtr, Channel) == TRUE) {
