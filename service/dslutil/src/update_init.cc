...
namespace service {
  namespace dslutil {
    namespace updater {
      ...
      /** @brief Initialize the updater DSL service
          Looks up the BSI information to pass to register_update
          Registers the updater service with DSL.
          Ownership of updater::Space is transferred to DSL.
      */
      void initialize() {
        Bsi bsi = LookupBsi();
        if (!bsi) {
          Logger().error("BSI lookup failed in service::dslutil::updater::init().");
          return;
        }
