...
int main (int argc, char* const argv[])
{
   ...
   for(i=optind; i<argc; ++i)
   {
	 ...
	 if(file)
	 {
	    ...
	    {
	       wr_rs = COB_rtm_eeprom_write(ctx, ipmb_addr, 0,
					  offset, len, data, wr_rs_buf);

	       if(wr_rs == NULL || wr_rs->ccode)
	       {
		  printf("ERROR - %s/%i: Failed to write to RTM EEPROM "
			 "offset 0x%x, length 0x%x\n",
			 shelf, slot, offset, len);
		  printf("ERROR - RTM FRU Information EEPROM may be "
	 ...
	 if(serial)
	 {
	    ...
	    id_rs = COB_fru_info_id_fixup(ctx, ipmb_addr, 
					  RTM_FRU_ID, id_rs_buf);
	    
	    if(id_rs == NULL || id_rs->ccode)
	    {
	       printf("ERROR - %s/%i: Failed to fill RTM FRU Info "
	 ...
	 if(tag[0])
	 {
	    ...
	    tag_rs = COB_fru_info_asset_tag_set(ctx, ipmb_addr, RTM_FRU_ID,
						tag, tag_rs_buf);

	    if(tag_rs == NULL || tag_rs->ccode)
	    {
	       printf("ERROR - %s/%i: Failed to fill RTM FRU Info Asset Tag\n",
		      shelf, slot);
	       ret = 1;
	    }
	 }
...
{
   printf("\n"
	  "Usage is: cob_rtm_init [OPTIONS] <rce_address> [<rce_address> ...]\n"
	  "\n"
